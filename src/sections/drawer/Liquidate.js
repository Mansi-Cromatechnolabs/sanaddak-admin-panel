...
export default function Liquidate({ open, onClose, data, onLiquidateClick }) {
  ...
  const areAllInstallmentsPaid = data?.installments?.every((emi) => emi.emi_status === 'paid');

  const lastInstallment = data?.installments[data?.installments.length - 1];
  const lastPaymentDate = new Date(lastInstallment.emi_payment_date);
  const today = new Date();
  const isLastPaymentDateAfterToday = lastPaymentDate < today;
  const shouldDisplayButton = isLastPaymentDateAfterToday;
  return (
    <div>
            ...
            >
              <CardContent>
                ...
                <Box
                  ...
                  mb={1}
                >
                  <Typography sx={{ color: (theme) => theme.palette.text.gray500, fontSize: 14 }}>
                    Liquidation Cost (%)
                  </Typography>
                  <Typography className="text-14 fw-bold">{data?.liquidation_cost_rate}</Typography>
                </Box>
                <Box
                  component="div"
                  display="flex"
                ...
                >
                  <Typography sx={{ color: (theme) => theme.palette.text.gray500, fontSize: 14 }}>
                    Liquidation Cost Amount
                  </Typography>
                  <Typography className="text-14 fw-bold">{data?.liquidation_costs}</Typography>
                </Box>
                <Box
                  component="div"
                  display="flex"
                ...
                >
                  ...
                  <Typography className="text-14 fw-bold">{data?.gold_piece_value}</Typography>
                </Box>
                <Box
                  component="div"
                  display="flex"
                ...
                >
                  <Typography sx={{ color: (theme) => theme.palette.text.gray500, fontSize: 14 }}>
                    Net Gold Liquidation Value
                  </Typography>
                  <Typography className="text-14 fw-bold">
                    {data?.new_gold_liquidation_value}
                  </Typography>
                </Box>
                <Box component="div">
                  ...
                  {data?.total_liqquidate_penalty.replace(/[^\d.-]/g, '') > 0 && (
                    <Box
                      component="div"
                      display="flex"
                      justifyContent="space-between"
            ...
            >
              <CardContent>
                <Typography
                  ...
                  gutterBottom
                >
                  TOTAL LIQUIDATE AMOUNT
                </Typography>
                <Typography variant="h6" color={(theme) => theme.palette.text.gray500} gutterBottom>
                  You can liquidate your gold and receive a total amount of:
                </Typography>
                <Typography variant="h2" mt={3}>
                  {areAllInstallmentsPaid
                    ? data?.funds_due_to_customer
